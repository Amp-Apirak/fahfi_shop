      ...
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div v-if="editModalPending" class="text-center"><div class="spinner-border"></div></div>
            ...
            <div class="row" v-if="!editModalPending && !editModalError">
              <div class="col-md-5" style="max-height: 60vh; overflow-y: auto;">
                ...
                >
                  {{ product.name }} ({{ product.sell_price }} บ.) - สต็อก: {{ product.stock_quantity }}
                </div>
              </div>
              ...
              <div class="col-md-7">
                ...
                <div v-if="editCart.length === 0" class="text-center text-muted">- ตะกร้าว่าง -</div>

                <ul class="list-group" style="max-height: 60vh; overflow-y: auto;">
                  <li v-for="(item, index) in editCart" :key="index" class="list-group-item">
                    <div class="d-flex w-100 justify-content-between">
                      <h6 class="mb-1">{{ item.name }}</h6>
                      <button class="btn-close btn-sm" @click="removeFromEditCart(index)"></button>
                    </div>
                    <div class="d-flex align-items-center">
                      <span class="me-2">จำนวน:</span>
                      <input type="number" class="form-control form-control-sm me-2" v-model.number="item.quantity" style="width: 70px;" min="1">
                      <span class="me-2">ส่วนลด (บ.):</span>
                      <input type="number" class="form-control form-control-sm" v-model.number="item.discount_amount" style="width: 90px;" min="0">
                    </div>
                  </li>
                </ul>
              </div>
...
// 8. (Modal 2) ฟังก์ชัน "ยกเลิกบิล" (คืนสต็อก)
...
const openEditModal = async (saleId) => {
  ...
  try {
    ...
    const saleRes = await axios.get(`http://localhost:3001/api/sales/${saleId}`, {
      headers: { 'Authorization': `Bearer ${token.value}` }
    });
